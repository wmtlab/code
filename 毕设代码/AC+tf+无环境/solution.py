...
class Critic(object):
    ...
    def learn(self, s, r, s_):
        ...
        return td_error
#########################################################################################################
EPISODE = 10 # change every EPISODE times每次选择20个样本训练一次模型？原20
N_F = EPISODE * 3 # speed,losepacket,application_speed
N_A = 3 # +10,0,-10
Lambda_init = 1.0 # random choose
random_counter_init = 50 # decline Lambda after random_counter原100
MAX_BANDWITH = 15000 # standardlize to 1

sess = tf.Session()   #相当于开关

actor = Actor(sess, n_features=N_F, n_actions=N_A, lr=LR_A)
critic = Critic(sess, n_features=N_F, lr=LR_C)     # we need a good teacher, so the teacher should learn faster than the actor  LR_C>LR_A

sess.run(tf.global_variables_initializer())   #Session 是 Tensorflow 为了控制,和输出文件的执行的语句. 运行 session.run() 可以获得你要得知的运算结果, 或者是你所要运算的部分.
...
class RL(CongestionControl):
    ...
    def __init__(self):
        ...
        for i in range(EPISODE):                                         #状态s的三段使用last_state存（不懂为什么用三次for循环）
            self.last_state.append(40.0/MAX_BANDWITH)    #归一化发送速率存进last state
    ...
    def estimate_bandwidth(self, cur_time, event_info):     #定义函数用来控制发送时的传送速率
        ...
        if self.counter == EPISODE:                     # 进来20个包之后？开始选择动作？
            ...
            self.random_counter-=1                    #前100没问题？，Lambda不变
            if self.random_counter <= 0:            #当递减到0时，开始衰减Lambda，减半到0.05以下时一直0.05（Lambda什么作用不知道）
                self.Lambda /=2.0
                if self.Lambda < 0.05:
                    self.Lambda = 0.05
            ...
            # reward
            r = 0                                                  #计算该20个的奖励，当没有丢包时+send_rate，丢包时候-send_rate * EPISODE
            for i in range(EPISODE):
            ...
            a = actor.choose_action(s_array)          #使用choose_action（当前状态）去选择动作a
            if random.random() < self.Lambda:      #训练刚开始的时候，我们的Critic效果较差此时，应该将λ设计得比较大。简单地说，当我们“主观”的估计不准的时候，我们应该用更多“客观”的数据去估计，而当训练已经经历了较多个iteration之后，Critic的偏差比较小，则可以将λ适当降低
            ...
            td_error = critic.learn(s, r, s_array)            #critic学习返回一个误差评价，用于之后actor使用

            # debug
            sums = 0
            for i in range(EPISODE,2*EPISODE):         #丢包数laststate
                sums += self.last_state[i]
            print("last_state:",self.last_state[0] * MAX_BANDWITH,sums)      #发送20个速率之和
            sums = 0
            for i in range(EPISODE,2*EPISODE):
                sums += s_[i]                                       #丢包数presentstate
            ...
            actor.learn(s, a, td_error)                          #actor根据状态s，动作a以及critic返回的评价td进行学习

            self.last_state = s_                                   #将当前状态转为上个状态并且清空resultlist
            self.result_list = []
